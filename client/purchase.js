...
/**
* Build payment details based on event price.
*
* @param {String} eventId Id of the item being purchased.
* @param {Object} shipping Selected shipping option.
* @returns {Object} paymentDetails passed into the SDK payment request.
*/
const paymentDetails = (eventId, shipping) => {
  ...
  return {
    displayItems: [
      ...
      },
      {
        ...
        amount: {
          currency: 'USD',
          value : calculateTax(event.price)
        },
      },
     shipping
    ],
    total: {
      ...
      amount: {
        currency: 'USD',
        value : calculateTotal(event.price, 0.0)// shipping.amount.value)
      },
    },
 ...
 * @returns {undefined}
 */
const thankForPurchase = (eventId, userId) => {
  fetch(`/users/${userId}/purchase/${eventId}`, {
    method: 'PUT',
  }).then((response) => {
    if (response.ok) {
 ...
 * @returns {undefined}
 */
const buyItem = (eventId, userId) => {

  var currentShippingOption = 'express';
  var details = paymentDetails(eventId, expressShipping);
  ...
  request.addEventListener('shippingaddresschange', function(evt) {
    evt.updateWith(new Promise(function(resolve, reject) {
      ...
      if (currentShippingOption === 'standard') {
        details = paymentDetails(eventId, standardShipping);
      } else {
        details = paymentDetails(eventId, expressShipping);
      }
      resolve(details);
    }));
  });
    ...
    if (response === true) {
      ...
      request.show().then(function(paymentResponse) {
        paymentResponse.complete('success').then(function() {
          logger.fbLog('payment_step', {step: "complete", event_id: eventId}, userId);
          // send a confirmation bubble into the thread.
          thankForPurchase(eventId, userId);
        });
